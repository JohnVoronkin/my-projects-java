  ...
  class SenderM {
    ...
    public void sendM(byte[] aData) {
      ...
      sender.send(aData);
       ...
       /*
       Обратите внимание, композиция подразумевает, что при уничтожении объемлющего объекта, уничтожается и содержимый в нем объект, это отличает композицию от агрегирования.
       В случае агрегирования, при уничтожении объемлющего объекта, содержимый объект может существовать дальше).
        */
  }
